...
연락처는 sangwookp9591@gmail.com.`,
  ...
  {
    id: 'codex-review',
    title: 'Codex가 평가한 박상욱',
    tags: 'codex 코덱스 평가 리뷰 관찰 티켓 모호한문제 경계 원인 운영가능 문제추적 풀스택 표현',
    text: `Codex는 박상욱을 “주어진 티켓만 구현하는 개발자는 아니다. 모호한 문제를 받으면 경계를 넓혀가며 원인을 찾고, 결국 운영 가능한 형태까지 만들어 놓는 개발자”라고 평가했습니다.
“풀스택 개발자”는 틀린 표현은 아니지만 너무 흔해서 특징이 사라진다고 보았습니다.
박상욱의 특징은 “프론트도 하고 백엔드도 한다”가 아니라 “문제가 어디로 넘어가든 따라간다”는 쪽이라고 평가했습니다.`,
  },
  {
    id: 'claude-review',
    title: 'Claude가 평가한 기술적 특징',
    tags: 'claude 클로드 평가 리뷰 관찰 원인분석 근본원인 테크리드 확장성 깊이 검증 공식문서 valkey ttl',
    text: `Claude는 박상욱이 문제를 증상이 아니라 원인 층위에서 잡는다고 평가했습니다. Valkey 메모리 문제에서 700만 키 중 73%에 TTL이 없다는 사실을 실측하고 세 발생원으로 나눈 뒤, 실행 가능한 핸드오프까지 만든 흐름을 근거로 들었습니다.
14개 로케일과 확장 가능한 데이터 구조를 사업의 다음 단계에 맞춰 설계하는 테크리드의 사고 패턴도 보았습니다.
백엔드·프론트·Flutter·AWS·OpenSearch까지 범위가 넓지만 각 영역의 트레이드오프를 설명할 만큼 깊게 들어간다고 평가했습니다.
추론보다 검색과 공식 문서, TDD·PRD·TASK.md처럼 검증 가능한 상태를 선호하는 점도 특징으로 꼽았습니다.`,
  },
  {
    id: 'claude-risk',
    title: 'Claude가 평가한 잠재적 리스크',
    tags: 'claude 클로드 평가 리스크 위험 약점 비용 병목 위임 문서화 출시속도 완결성 선제확장',
    text: `Claude는 선제 설계가 확장성을 얻는 대신 쿼리 복잡도, 인덱싱 난이도, 팀원 진입장벽을 높일 수 있다고 보았습니다. 그래서 문서화와 위임의 밀도가 중요하다고 평가했습니다.
인프라부터 프론트와 앱 배포까지 직접 해결하는 패턴이 반복되면, 가장 빠른 해결자인 본인이 조직의 병목이 될 수 있다고 지적했습니다.
완결성을 추구하는 성향은 장애를 줄이지만 시장 검증 단계에서는 “70%로 내보내고 배우기”와 균형을 맞춰야 한다고 평가했습니다.`,
  },
  {
    id: 'repos',
    title: '세 저장소에서의 포지션',
...
그 외: WebGPU/WGSL, three.js, 온디바이스 LLM(transformers.js), 멀티 LLM 프로바이더 연동, 이미지·영상 생성 파이프라인.`,
  },
  {
    id: 'site',
    title: '이 사이트',
    tags: '사이트 포트폴리오 이력서 복도 방 3d three r3f 아잉 aing 채팅 채팅창 브라우저모델 next 서버컴포넌트 읽기모드',
    text: `이 포트폴리오는 복도를 걷는 사이트입니다. 스크롤이 재생이 아니라 걸음이고,
복도 양옆에 사례가 하나씩 방으로 붙어 있어 문 앞에 서면 들어갈 수 있습니다.
여섯 방은 앱 없이 결제하는 웹, 운영자용 관리 화면, 장애가 번지지 않는 서버,
이전 프로젝트, 직접 만든 캐릭터와 에셋, 팀이 함께 쓰는 개발 도구입니다.
방마다 모니터가 하나 있고 거기 뜨는 건 그림이 아니라 실제 화면이라 눌러 보고 스크롤할 수 있습니다.
읽는 글은 언제나 HTML로 존재합니다 — 3D는 공간과 카메라만 맡고, '전부 읽기'를 누르면 3D 없이 문서로 읽힙니다.
Next.js 16 App Router에서 본문은 서버 컴포넌트로 렌더하고, 복도와 방은 React Three Fiber와 three.js로 세웠습니다.
지금 이 채팅은 브라우저 안에서 도는 소형 언어 모델(Gemma 3 1B, 약 763MB)이 답하고,
모델을 못 올리는 브라우저에서는 이 위키가 그대로 답합니다. 어느 쪽이든 질문은 서버로 나가지 않습니다.`,
  },
  {
    id: 'aingkit',
    title: '아잉 — 직접 만든 캐릭터와 에셋 킷',
    tags: '아잉 aing 마스코트 캐릭터 에셋 스프라이트 표정 동작 포즈 모션 애니메이션 glb 3d모델 그림 디자인 색',
    text: `사이트를 안내하는 캐릭터 아잉은 장식으로 가져다 쓴 그림이 아니라 직접 만든 에셋 킷입니다.
표정 16종, 동작 16종, 움직이는 모션 6종을 갖췄고, 웹에서 쓸 수 있는 3D 모델도 두 벌 만들었습니다.
가벼운 쪽은 205KB입니다.
256px 셀의 4×4 스프라이트 시트와 목록 파일을 함께 두어 필요한 표정과 동작을 이름으로 찾아 쓸 수 있게 했습니다.
아잉의 색 다섯 가지를 먼저 정하고 방의 벽과 카드에도 같은 색표를 썼습니다.
배경을 지운 뒤 남은 작은 얼룩은 걸러 내고 캐릭터의 선과 넓은 면은 보존했습니다.
움직임은 앞뒤를 이어 붙여 처음과 끝이 뚝 끊겨 보이지 않게 했습니다.
다른 사람이 가져다 쓸 수 있도록 킷을 통째로 공개했습니다.`,
  },
  {
    id: 'career',
