...
- 킥커: `7년차 풀스택 개발자 · 박상욱 (iron)`
- 제목(h1): `14개 언어 의료관광 웹과,` / `운영 화면·서버를 만들었습니다.` (두 줄)
- 본문: 의료관광 서비스 ZIVO에서 환자가 쓰는 웹, 운영자가 매일 쓰는 관리 화면, 그 뒤의 Spring Boot 서버를 한 흐름으로 맡았습니다. 결제 이탈과 외부 장애처럼 정상 화면 밖에서 생기는 문제를 찾고, 같은 실수가 반복되지 않게 고칩니다.
- CTA: `핵심 사례 3개 보기` / `메일 보내기`
- 기본 화면은 한눈에 읽는 문서이며, `3D 복도 둘러보기`를 누르면 복도 탐색으로 전환됩니다.
...
| 15명 | 같은 권한과 공통 화면으로 협업한 규모 |
| 3개 영역 | 환자용 웹·운영 화면·서버를 맡은 범위 |

### 핵심 사례 3개 요약

첫 화면 다음에 ZIVO의 세 경계를 같은 형식으로 비교합니다.

1. **환자용 웹** — 앱 설치 없이 검색부터 결제까지 · `14개 언어 · 9개월 운영 · 웹 0→1 단독 구축`
2. **운영자 화면** — 15명이 같은 권한과 화면을 사용하도록 · `15명 협업 · 권한·공통 UI 기준 일원화`
3. **백엔드** — 외부 장애와 중복 지급이 번지지 않도록 · `외부 연동 43곳 격리 · 쿠폰 검증 101건`
...
> “주어진 티켓만 구현하는 개발자는 아니다.
> 모호한 문제를 받으면 경계를 넓혀가며 원인을 찾고,
> 결국 운영 가능한 형태까지 만들어 놓는 개발자다.”

가장 어울리지 않는 표현은 의외로 “풀스택 개발자”일 수 있습니다. 틀린 표현은 아니지만 너무 흔해서 특징이 사라집니다.

“프론트도 하고 백엔드도 한다”가 아니라 **“문제가 어디로 넘어가든 따라간다”** 쪽입니다. 그것이 Codex가 본 박상욱의 가장 뚜렷한 특징입니다.

### Claude의 평가 — 기술적으로 보이는 특징

1. **문제를 증상이 아니라 원인 층위에서 잡는 편**
   Valkey 메모리 이슈에서 인스턴스 증설부터 택하지 않았습니다. 700만 키 중 73%가 TTL이 없다는 사실과 volatile-lru 정책의 조합을 확인하고, Onda 동기화 배치·Google Places 캐시·재고 날짜 키라는 세 발생원으로 나눴습니다. 진단 → 근본원인 → 재발방지 → 실행 가능한 산출물의 사이클을 습관적으로 돈다는 평가입니다.

2. **설계를 지금이 아니라 2년 뒤에 맞춰 두는 성향**
   14개 로케일 다국어 테이블, 언어·국가의 분리, Generic FK 폴리모픽, 복합 커서 페이지네이션처럼 아시아 시장 확장과 스키마 설계를 맞춥니다. 개발자보다 테크리드에 가까운 사고 패턴이라는 평가입니다.

3. **범위가 넓은데 깊이가 얕지 않음**
   Spring Boot/Kotlin, React 19와 FSD, Flutter, ECS Fargate와 RDS Multi-AZ, OpenSearch, 카테고리 분류 체계까지 다루면서 각 영역의 트레이드오프를 설명하는 수준으로 들어갑니다.

4. **검증되지 않은 것을 싫어함**
   검색과 공식 문서를 요구하고, TDD·PRD·TASK.md처럼 검증 가능한 상태를 만들어 움직입니다. LLM의 그럴듯한 답도 검증 전에는 리스크로 본다는 평가입니다.

### Claude의 평가 — 잠재적 리스크

- **선제 설계의 비용** — 확장성을 사는 대신 쿼리 복잡도·인덱싱 난이도·팀원 진입장벽을 지불할 수 있습니다. 설계 실력만큼 문서화와 위임의 밀도가 중요합니다.
- **직접 다 파고드는 성향의 한계** — 인프라부터 프론트와 앱 배포까지 직접 잡는 패턴이 반복되면 가장 빠른 해결자인 본인이 조직의 병목이 될 수 있습니다.
- **완결성 욕구와 출시 속도** — A부터 Z까지 확인하는 성향은 장애를 줄이지만, 시장 검증 단계에서는 70%로 내보내고 배우는 편이 정답일 때도 있습니다.
